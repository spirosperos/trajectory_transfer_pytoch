...
struct RobotParams
{
    ...
    const double day = 0;
    const double daz = 0;
    const double dbz = -0.0297-0.034-0.0468;
...
*/
    ...
    double sp7y = sp7y_;
    double sp7z = sp7z_;
    const double K__a=60000;
    const double K__b=60000;
  ...
  //Gear ratios for each link
     const double n__a=160;
     const double n__b=160;
     ...
     const double I__raxx=0.000185;
     const double I__rayy=I__r1xx;
     const double I__razz=0.000185+0.000238166953125;
     const double I__rbxx=0.000185;
     const double I__rbyy=I__r1xx;
     const double I__rbzz=0.000185+0.000238166953125;
   ...
   //Moments of inertia of the rotors
     const double B[9] = {n__a*n__a*I__razz,n__b*n__b*I__rbzz,n__1*n__1*I__r1zz, n__2*n__2*I__r2zz,n__3*n__3*I__r3zz,n__4*n__4*I__r4zz,n__5*n__5*I__r5zz,n__6*n__6*I__r6zz, n__7*n__7*I__r7zz};
    const double deg_to_rad = M_PI / 180.0;
    //const std::array<double, 9> tau_max = {320, 320, 320, 320, 176, 176, 110, 40, 40};
    const std::array<double, 9> Tau_lb = {-800, -1000, -320, -320, -176, -176, -110, -40, -40};
    const std::array<double, 9> Tau_ub = {800, 1000, 320, 320, 176, 176, 110, 40, 40};
    const std::array<double, 9> q_limit_upper = {(1.3930-0.3000/2)+1, (0.9725-0.2200/2), 170 * deg_to_rad, 120 * deg_to_rad, 170 * deg_to_rad, 120 * deg_to_rad, 170 * deg_to_rad, 120 * deg_to_rad, 175 * deg_to_rad};
    const std::array<double, 9> q_limit_lower = {-(1.3930-0.3000/2)+1, -(0.9725-0.2200/2), -170 * deg_to_rad, -120 * deg_to_rad, -170 * deg_to_rad, -120 * deg_to_rad, -170 * deg_to_rad, -120 * deg_to_rad, -175 * deg_to_rad};
};
